...
with open("Advent of Code\\AoC2022\Day 3\input.txt", "r") as file:
    for line in file:
        contents = line
...
with open("Advent of Code\\AoC2022\Day 3\input.txt", "r") as file:
    for line in file:
        ...
        if group_counter == 2:
            for x in lineList[0]:
                if (x in lineList[1]) and (
                    x in lineList[2]
                ):  # made a mistake here, wrote 'x in a[1] and a[2]' (the latter of which is always true here)
                    if (ord(x) >= 65) and (ord(x) <= 90):
                        priority_sum += ord(x) - 38
                        break
                    elif (ord(x) >= 97) and (ord(x) <= 122):
